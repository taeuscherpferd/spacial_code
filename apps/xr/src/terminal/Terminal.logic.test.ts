...
describe('TerminalLogic', () => {
  it('keeps scrollback and handles carriage-return progress', () => {
    let buffer = TerminalLogic.consume(TerminalLogic.create(), 'Loading 1%\rLoading 2%\nDone')
  ...
  it('preserves lines written with Windows CRLF endings', () => {
    ...
    expect(buffer.lines.map((line) => line.spans[0]?.text)).toEqual(['one', 'two'])
  })

  it('filters terminal title sequences across chunks', () => {
    let buffer = TerminalLogic.consume(TerminalLogic.create(), '\u001b]0;Spatial')
  ...
  it('returns a virtualized scrollback window', () => {
    const buffer = TerminalLogic.consume(TerminalLogic.create(), 'one\ntwo\nthree')
